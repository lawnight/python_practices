...
class JDSpider:
    ...
    def get_avilable_id(self,skuids, area):
        """获取可购买的商品id"""
        ...
        nohasSkuid = []
        unUseSkuid = []
        for sku_id, info in self.parse_json(resp.text).items():
        ...
        logging.info('检测[%s]个口罩有货，[%s]个口罩无货，[%s]个口罩下柜，耗时[%s]ms', len(inStockSkuid), len(nohasSkuid), len(unUseSkuid),
                    int(time.time() * 1000) - start)

        if len(unUseSkuid) > 0:
            logging.info('[%s]口罩已经下架', ','.join(unUseSkuid))
            #移除已下架的商品，不用再去监控
            for id in unUseSkuid:
                skuids.remove(id)
            logging.info('[%s]已经移除下架商品', ','.join(unUseSkuid))
        return inStockSkuid
