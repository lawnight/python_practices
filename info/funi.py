...
'''
蓝光每日成交抓取
透明网的抓取，最先通过web网页，web网页端使用swf实现的。先通过burp suite来分析协议。需要在设置中开启对amf的支持。发现amf的交互，都做了加密。需要破解swf文件。难度大。
...
def getInfo():
    ...
    info['totalArea'] =  obj['data']['totalArea']
    info['infoDate'] =  obj['data']['date']
    info['totalPrice'] =  obj['data']['totalPrice']
    info['date'] = getTime()
