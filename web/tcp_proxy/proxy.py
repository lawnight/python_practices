...
class HttpParser(object):
        ...
        if self.state >= HTTP_PARSER_STATE_HEADERS_COMPLETE and \
            ...
            if b'content-length' in self.headers:
                ...
                if self.chunk_parser.state == CHUNK_PARSER_STATE_COMPLETE:
                    ...
                    self.state = HTTP_PARSER_STATE_COMPLETE
            if b'content-encoding' in self.headers and self.headers[b'content-encoding'][1].lower() == b'gzip':
                # 解压gzip
                self.body = zlib.decompress(self.body,wbits= zlib.MAX_WBITS | 16)
                self.body = self.body.replace(b'114.116.11.81',b'192.168.202.120')
                ...
                hexdump.hexdump(self.modifyRaw)
            return False, b''
    ...
    def build_response(self, del_headers=None, add_headers=None):
        """build client request """
        req = b' '.join([self.version,b'200 '])
        req += CRLF
